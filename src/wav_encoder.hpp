...
public:
    //------------------------------------------------------------------------------------------
    wav_encoder(const char *filename, uint32_t numChannels, uint32_t sampleFrequency, uint32_t wavFormat, uint32_t bytesPerSample)
    {
        drwav_data_format format;
        format.container        = drwav_container_riff; // <-- drwav_container_riff = normal WAV files, drwav_container_w64 = Sony Wave64.
        format.format           = wavFormat;            // <-- Any of the DR_WAVE_FORMAT_* codes.
